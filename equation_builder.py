...
def equation_builder( components, imported_bc_equations = None,  printing = 'off' ):
    ...
    ************************************************************************************************************
    ...
    for reaction_rate in reaction_rates:                    # Now we will go through each reaction to write the reaction rate equation for it
        ...
        if len( reaction_rate.id().split('_') ) == 3:

            if reaction_rate.id().split('_')[2] == 'nma':
    ...
    for bc in boundary_conditions:
        ...
        bc_chebis = bc.id().split('_')[1].split('.')
    ...
    for v_item in variables:
        ...
        for bc in boundary_conditions:
            ...
            compounds = bc_id.split('_')[1].split('.')        # Here I get the compound name or chebi code
...
def equation_reader( components, equations_path, imported_bc_equations = None,  printing = 'off' ):
    ...
    ************************************************************************************************************
    ...
    imported_rate_equations = eft.read_equations_from_file( equations_path )

    for reaction_rate in reaction_rates:

        reaction_name = reaction_rate.name()

        for eq in imported_rate_equations:

            if str( eq.lhs ) == reaction_name:

                rhs = eq.rhs

                eq_symbols = rhs.free_symbols

                for parameter in eq_symbols:

                    if str( parameter ) in general_equations:

                        rhs = rhs.subs( parameter, general_equations[str(parameter)] )

                reaction_rate_equations_dict[reaction_rate.name()] = rhs
                reaction_rate_equations.append( eq )
